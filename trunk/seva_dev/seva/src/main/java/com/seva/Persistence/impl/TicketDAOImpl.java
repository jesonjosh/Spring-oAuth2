...
public class TicketDAOImpl extends AbstractDAO implements TicketDAO {
	...
	public List<TicketDTO> getTickets() {
		...
		Query query = session.createSQLQuery(" SELECT TKT.ID,TKT.CREATE_DATE,TKT.ACTIVE_DATE,TKT.CREATION_HOUR,TKT.PAID,TTN.TABLE_ID "
		+ " FROM TICKET AS TKT "
		...
		for(Object object: ticketObjects){
			...
			Query itemQuery = session.createSQLQuery("SELECT TIT.ITEM_ID,TIT.ITEM_COUNT,TIT.ITEM_NAME,TIT.GROUP_NAME,TIT.CATEGORY_NAME, TIT.ITEM_PRICE"
			+ " FROM TICKET_ITEM AS TIT "
			+ " WHERE TIT.TICKET_ID =:ticket_id").setParameter("ticket_id", ticketDTO.getId());
